 ...
 * список/множество объектов и подсчитать количество треугольников 
 * разного типа (равносторонний, равнобедренный, прямоугольный, произвольный). 
 * Определить для каждой группы наибольший и наименьший по 
 * площади (периметру) объект.
 */
...
public class Main {
  ...
  private static void Task3() {
    triangles.add(new Triangle(new Triangle.Point(0, 0), new Triangle.Point(1, 0), new Triangle.Point(0.5, Math.sqrt(3) / 2))); // Равносторонний
    triangles.add(new Triangle(new Triangle.Point(0, 0), new Triangle.Point(1, 0), new Triangle.Point(1, 1))); // Прямоугольный
    triangles.add(new Triangle(new Triangle.Point(0, 0), new Triangle.Point(1, 1), new Triangle.Point(2, 0))); // Произвольный
    triangles.add(new Triangle(new Triangle.Point(0, 0), new Triangle.Point(2, 0), new Triangle.Point(1, 1))); // Равнобедренный
    triangles.add(new Triangle(new Triangle.Point(1, 1), new Triangle.Point(3, 1), new Triangle.Point(2, 3))); // Равнобедренный
    triangles.add(new Triangle(new Triangle.Point(0, 0), new Triangle.Point(2, 0), new Triangle.Point(1, 0))); // Вырожденный (площадь = 0)

    Map<String, List<Triangle>> trianglesByType = new HashMap<>();
    for (Triangle triangle : triangles) {
      trianglesByType.computeIfAbsent(triangle.getType(), e -> new ArrayList<>()).add(triangle);
    }
